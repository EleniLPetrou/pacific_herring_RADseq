...
# Linear regression
regression_allpops_IBD = lm(pairwise_fst ~ diff_in_days, data = fst_data)
...
regression_IBD_Mar_Apr_primary = lm(pairwise_fst ~ diff_in_days, data = primary)
...
# Populate the empty arrays with data saved in the vectors
dist_array[cbind(i,j)] <- dist_array[cbind(j,i)] <- primary$diff_in_days
...
late2late_regression2 = lm(pairwise_fst ~ diff_in_days, data = late)
...
# Populate the empty arrays with data saved in the vectors
dist_array[cbind(i,j)] <- dist_array[cbind(j,i)] <- late$diff_in_days
